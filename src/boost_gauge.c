...
/* Geometry constants */
#define DIAL_D 280            /* full bezel diameter       */
...
#define PAD_RADIAL -55        /* label inward offset      */
...
void create_boost_gauge(lv_obj_t *parent)
{
    ...
    */
    ...
    /* major ticks */
    lv_obj_set_style_line_width(scale_obj, 5, LV_PART_INDICATOR);
    lv_obj_set_style_length(scale_obj, 22, LV_PART_INDICATOR);
    lv_obj_set_style_line_color(scale_obj, lv_color_white(), LV_PART_INDICATOR);

    /* labels */
    lv_obj_set_style_text_font(scale_obj, &eurostile_extended_black_16, LV_PART_INDICATOR);
    ...
    lv_obj_set_size(inner, SCALE_D - 70, SCALE_D - 70);
    // Configuration
    ...
    lv_scale_set_angle_range(inner, 240);
    ...
    lv_obj_set_style_text_font(inner, &eurostile_extended_black_10, LV_PART_INDICATOR);
    ...
    lv_obj_set_style_text_font(l, &eurostile_extended_black_10, 0);
    lv_obj_set_style_text_color(l, lv_color_hex(0xB10000), 0);
    float ang = (225.0f - ((30 + 10) * (270.0f / 40.0f))) * (float)M_PI / 180.0f;
    int cx = DIAL_D / 2, cy = DIAL_D / 2;
    float r = (SCALE_D / 2.0f) + 8.0f;
    int x = (int)(cx + r * cosf(ang)) - lv_obj_get_width(l) / 2 - 50;
    int y = (int)(cy + r * sinf(ang)) - lv_obj_get_height(l) / 2 + PSI_Y_OFF + 15;
    ...
    /* 4) “psi” label under 30 psi */
    {
        ...
        lv_obj_set_style_text_font(u, &eurostile_extended_black_12, 0);
        lv_obj_set_style_text_color(u, lv_color_hex(0xB10000), 0);
        float ang = (225.0f - ((30 + 10) * (270.0f / 40.0f))) * (float)M_PI / 180.0f;
        int cx = DIAL_D / 2, cy = DIAL_D / 2;
        float r = (SCALE_D / 2.0f) + 8.0f;
        int x = (int)(cx + r * cosf(ang)) - lv_obj_get_width(u) / 2 - 35;
        int y = (int)(cy + r * sinf(ang)) - lv_obj_get_height(u) / 2 + PSI_Y_OFF + 65;
...

    {
        ...
        lv_obj_set_style_text_font(b, &eurostile_extended_black_20, 0);
